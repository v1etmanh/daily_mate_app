...
const SearchModal = ({ visible, onClose, allIngredients, selectedIds, onToggle, loading }) => {
  ...
  return (
    <Modal visible={visible} animationType="slide" onRequestClose={onClose} statusBarTranslucent>
      <KeyboardAvoidingView style={{ flex:1, backgroundColor:'#FFFFF0' }}
        ...
        <View style={st.searchWrap}>
          ...
          <TextInput ref={inputRef} style={st.searchInput}
            placeholder="Tìm theo tên (vd: tôm, cua, sữa...)"
            placeholderTextColor="#9CA3AF" value={query} onChangeText={setQuery}
...
const AllergyScreen = () => {
  ...
  const [categories, setCategories] = useState(CATEGORY_LIST.map(c => ({ ...c, selected:false })));
  ...
  const preloadIngredients = async () => {
    setIngLoading(true);
    try { const list = await loadAllIngredients(); setAllIngredients(list); }
    catch (e) { Alert.alert('Oops!', 'Không tải được danh sách nguyên liệu 😅'); }
  ...
  const loadAllergiesFromDB = async () => {
    try {
      ...
      if (ingRows.length > 0) {
        setSelectedIngredients(ingRows.map(r => ({ id:r.allergy_key, name:r.display_name, name_en:'' })));
        if (catKeys.length === 0) setMode('ingredient');
      }
      setStoreAllergies([...catKeys, ...ingRows.map(r => r.allergy_key)]);
    } catch (e) { console.error('loadAllergiesFromDB:', e); }
  };

  const syncStore = (cats, ings) => {
    setStoreAllergies([...cats.filter(c=>c.selected).map(c=>c.key), ...ings.map(i=>String(i.id))]);
  };
  ...
  const toggleCategory = async (key) => {
    ...
    } catch (e) { Alert.alert('Oops!', 'Không thể cập nhật dị ứng 😅'); }
  };
  ...
  const toggleIngredient = useCallback(async (item) => {
    ...
    try {
      if (isSelected) {
        ...
        setSelectedIngredients(updated); syncStore(categories, updated);
      } else {
  ...
  const removeIngredient = useCallback(async (idStr) => {
    try {
      ...
      const updated = selectedIngredients.filter(i => String(i.id) !== idStr);
      setSelectedIngredients(updated); syncStore(categories, updated);
    } catch (e) { Alert.alert('Oops!', 'Không thể xóa nguyên liệu 😅'); }
  }, [selectedIngredients, categories]);

  const selectedIdSet = useMemo(
    () => new Set(selectedIngredients.map(i => String(i.id))), [selectedIngredients]
  );
  ...
  return (
    <View style={st.root}>
        ...
        {mode === 'ingredient' && (
          <View style={st.section}>
            ...
            {selectedIngredients.length > 0 ? (
              <View style={st.tagsWrap}>
                {selectedIngredients.map(i => (
                  <IngredientTag key={String(i.id)} name={i.name}
            ...
            <TouchableOpacity style={st.addBtn}
              onPress={() => { if (allIngredients.length===0) preloadIngredients(); setModalVisible(true); }}
              activeOpacity={0.8}>
      ...
      <SearchModal visible={modalVisible} onClose={() => setModalVisible(false)}
        allIngredients={allIngredients} selectedIds={selectedIdSet}
        onToggle={toggleIngredient} loading={ingLoading} />
    </View>
  );
};
...
const st = StyleSheet.create({
  ...
  header:        { backgroundColor:'#FFFFFF', padding:20, paddingTop:16,
                   borderBottomWidth:2, borderBottomColor:'#E5E7EB', borderStyle:'dashed' },
  title:         { fontSize:32, fontFamily:'Patrick Hand', color:'#1E1E1E' },
  subtitle:      { fontSize:16, fontFamily:'Nunito', color:'#9CA3AF', marginTop:4, lineHeight:22 },

  modeRow:       { flexDirection:'row', margin:16, backgroundColor:'#F3F4F6',
  ...
  catItem:       { flexDirection:'row', alignItems:'center', backgroundColor:'#FFFFFF',
  ...
  tag:           { flexDirection:'row', alignItems:'center', backgroundColor:'#EFF6FF',
                   borderRadius:9999, paddingVertical:6, paddingLeft:12, paddingRight:8,
                   borderWidth:1, borderColor:'#60A5FA' },
  tagText:       { fontSize:14, fontFamily:'Nunito', color:'#3B82F6', fontWeight:'600', maxWidth:140 },
  ...
  addBtn:        { backgroundColor:'#60A5FA', borderRadius:16, paddingVertical:14,
  ...
  modalHeader:   { flexDirection:'row', alignItems:'center', backgroundColor:'#FFFFFF',
                   paddingTop:Platform.OS==='ios'?56:20, paddingBottom:14,
                   paddingHorizontal:16, borderBottomWidth:2, borderBottomColor:'#E5E7EB', borderStyle:'dashed' },
  modalBack:     { width:60 },
  modalBackText: { fontSize:16, fontFamily:'Nunito', color:'#60A5FA', fontWeight:'700' },
  modalTitle:    { flex:1, textAlign:'center', fontSize:18, fontFamily:'Nunito', fontWeight:'700', color:'#1E1E1E' },

  searchWrap:    { flexDirection:'row', alignItems:'center', backgroundColor:'#FFFFFF',
                   marginHorizontal:16, marginTop:14, marginBottom:4, borderRadius:16,
                   paddingHorizontal:12,
                   shadowColor:'#000', shadowOffset:{width:0,height:1}, shadowOpacity:0.06, shadowRadius:3, elevation:2 },
  searchIcon:    { fontSize:16, marginRight:8 },
  searchInput:   { flex:1, fontSize:18, fontFamily:'Nunito', color:'#1E1E1E', paddingVertical:12 },
  searchHint:    { fontSize:14, fontFamily:'Nunito', color:'#9CA3AF', textAlign:'center', marginBottom:8, marginTop:2 },

  srItem:        { flexDirection:'row', alignItems:'center', backgroundColor:'#FFFFFF',
  ...
  emptyText:     { textAlign:'center', color:'#9CA3AF', marginTop:40, fontSize:16, fontFamily:'Nunito' },
});
